...
from taintlog_json import CallActionLogEntry, CipherUsageLogEntry, FileSystemLogEntry, NetworkSendLogEntry, SSLLogEntry, SendSmsLogEntry, SimulatorActionLogEntry
from taintlog_json import AppReportEntry, MainReportEntry
from taintlog_json import JsonFactory
from threading import Thread
from detection.action_processor import ActionProcessor
...
class RunnerThread(Thread):
    ...
    def __init__(self, theTDRunnerMain, theApp, theLogger=Logger()):
        ...
        self.resultFetchedFlag = False # Flag for whether has fetched logcat
        self.logcatRedirectFile = '/mnt/sdcard/logcat.log'
    ...
    def __checkForCancelation(self):
        ...
        """
        if self.cancelFlag:
            self.log.write('%s: Cancelation flag found, abort thread' %
                           Utils.getTimeAsString(datetime.datetime.now()))
            traceback.print_stack(file=self.log.log)
            raise KeyboardInterrupt

    def getResult(self):
        #in case thread is not responding, fetch it ourselves
        if self.resultFetchedFlag == False and self.emulator is not None and self.emulator.isEmulatorRunning():
            self._buildLogEntry()                                                                         
        return self.result
    ...
    def run(self):
        ...
        """
        # Debug infos
        self.log.debug('simulationSteps: %s' %
                       SimulationSteps.getStepsAsString(self.simulationSteps))
        ...
        try:
            imageDirPath = None
            self.log.write('Analyze app %s (%s)' %
                           (self.app.getApkFileName(), self.app.getApkPath()))
            ...
            imageDirPath = self._initCleanImageDir(
                self.tdRunnerMain.imageDirPath, self.app.getId(), self.app.getApkName())

            # Init result
            # if exception thrown, result may not be properly initialized,
            # resulting in error
            ...
            self.emulator = EmulatorClient(
                theSdkPath=self.tdRunnerMain.sdkPath,
            ...
            # Run app
            keyboardInterruptFlag = self.runApp(
                self.emulator, self.app, self.simulationSteps)

            # Stop emulator
            self.emulator.stop()

            # Print results
            self.log.write('- App: %s (package: %s)' %
                           (self.app.getApkFileName(), self.app.getPackage()))
            self.log.write(
                '- Time: %s %s - %s' % (Utils.getDateAsString(self.startTime),
                                        Utils.getTimeAsString(self.startTime), Utils.getTimeAsString(self.result['endTime'])))
            if 'log' in self.result:
            ...
            traceback.print_exc()
            self.log.error("error in emulator")
            ...
            if self.tdRunnerMain.cleanUpImageDir:
                ...
                self.log.info(
                    'Image dir \'%s\' will not be removed, cleanUpImageDir flag set to false.' % imageDirPath)
    ...
    def _initCleanImageDir(self, theImageDir, theSampleId, theAppName):
        ...
        """
        imageFileList = [
            'ramdisk.img', 'sdcard.img', 'system.img', 'userdata.img', 'zImage']
        ...
        for imageFile in imageFileList:
            ...
            shutil.copy2(
                os.path.join(theImageDir, imageFile), os.path.join(newPath, imageFile))
        return newPath
    ...
    def _cleanUpImageDir(self, theImageDirPath):
        ...
        if not theImageDirPath is None:
            try:
                ...
                self.log.error(
                    'Error during cleaning up image dir \'%s\': %s' % (theImageDirPath, ose))
    ...
    def _getLogcatFileName(self, theLogcatDirPath, theSampleId, theFileName):
        ...
        logcatFileName = '%s%s_%06d_logcat.log' % (
            theLogcatDirPath, theFileName, theSampleId)
        self.result['logcatFileName'] = logcatFileName
        if os.path.exists(logcatFileName):
            i = 1
            while True:
                logcatFileName = '%s%s_%06d_%02d_logcat.log' % (
                    theLogcatDirPath, theFileName, theSampleId, i)
                if os.path.exists(logcatFileName):
                    i += 1
                else:
                    break
    ...
    def _storeLogcatAsFile(self, theLogcatDirPath, theSampleId, theFileName, theLog):
        ...
        """
        logcatFileName = self._getLogcatFileName(
            theLogcatDirPath, theSampleId, theFileName)
    ...
    def runApp(self, theEmulator, theApp, theSteps):
        ...
        if numMonkeyRuns != 0:
            if self.numMonkeyEvents % numMonkeyRuns == 0:
                ...
                numMonkeyEventsFirst = self.numMonkeyEvents / numMonkeyRuns
                numMonkeyEventsLast = self.numMonkeyEvents - \
                    (numMonkeyEventsFirst * (numMonkeyRuns - 1))

        # Clear log
        theEmulator.clearLog()
        self.result['simulatorActions'] = []

        # Install app
        self.log.write('- %s: Install app' %
                       Utils.getTimeAsString(datetime.datetime.now()))
        if (theSteps & SimulationSteps.INSTALL):
            ...
            while True:
                ...
                try:
                    ...
                    self.result['simulatorActions'].append(
                                                      SimulatorActionLogEntry.makeInstallAction(
                                                                                                theApp.getApk(),
                                                                                                Utils.getDateTimeAsString(datetime.datetime.now())
                                                                                                )
                                                      )
                    break
                    ...
                    if ecErr.getCode() == EmulatorClientError.INSTALLATION_ERROR_ALREADY_EXISTS:
                        ...
                        if numRetries == 4:
                            self.log.debug(
                                'Number of maximum retries reached, abort installation')
                        else:
                            # Wait and retry
                            errorOccuredFlag = False
                            waitTime = numRetries * 10
                            self.log.debug(
                                'Installation failed as system might not be running. Wait for %dsec and try again' % waitTime)
                            time.sleep(waitTime)
                            continue
        ...
        theEmulator.startLogcatRedirect(self.logcatRedirectFile, self.maxLogcatSize)

        # Switch on taint tracking
        #theEmulator.setProperty('tdroid.global.taintmask', '7176')
        theEmulator.changeGlobalTaintLogState('1', True)
        theEmulator.changeGlobalTaintLogFileSystemTaintMask('1')

        # Do not start all services in one time, which may cause ANR
        self.__checkForCancelation()
        ...
        try:
            ...
            if (theSteps & SimulationSteps.MONKEY_BEFORE_GSM):
                self._runMonkey(
                    theEmulator, theApp.getPackage(), numMonkeyEventsFirst)
            if (theSteps & SimulationSteps.GSM):
                self._runGsmSimulation(theEmulator.getTelnetClient(), self.result['simulatorActions'])
            if (theSteps & SimulationSteps.MONKEY_BEFORE_GEO):
                self._runMonkey(
                    theEmulator, theApp.getPackage(), numMonkeyEventsFirst)
            if (theSteps & SimulationSteps.GEO):
                self._runGeoSimulation(theEmulator.getTelnetClient(),self.result['simulatorActions'])
            if (theSteps & SimulationSteps.MONKEY_BEFORE_SMS):
                self._runMonkey(
                    theEmulator, theApp.getPackage(), numMonkeyEventsFirst)
            if (theSteps & SimulationSteps.SMS):
                self._runSmsSimulation(theEmulator.getTelnetClient(), self.result['simulatorActions'])
            if (theSteps & SimulationSteps.MONKEY_BEFORE_POWER):
                self._runMonkey(
                    theEmulator, theApp.getPackage(), numMonkeyEventsFirst)
            '''some emulator crashes on power command, comment it out
            if (theSteps & SimulationSteps.POWER):
                self._runPowerSimulation(theEmulator.getTelnetClient())
            '''
            if (theSteps & SimulationSteps.MONKEY):
                self._runMonkey(
                    theEmulator, theApp.getPackage(), numMonkeyEventsLast)
    ...
    def _buildLogEntry(self):
        if self.resultFetchedFlag is True:
            return
        logAnalyzer = None
        try:
            theEmulator = self.emulator
            theApp = self.app
            self.log.write('- %s: Analyze log' %
                           Utils.getTimeAsString(datetime.datetime.now()))
            ...
            logcatFileName = self._getLogcatFileName(
                self.tdRunnerMain._getLogDirPath(), theApp.getId(), theApp.getApkName())
            theEmulator.storeLogcatRedirectFile(
                self.logcatRedirectFile, logcatFileName)
        ...
        self.result['endTime'] = datetime.datetime.now()
        self.resultFetchedFlag = True

    # ========================================================================
    # Simulations
    # ========================================================================
    def _runGsmSimulation(self, theTelnetClient, logResult):
        """
        Simulates incoming calls, log it
        """
        self.__checkForCancelation()

        self.log.info('- %s: GSM simulation' %
                      Utils.getTimeAsString(datetime.datetime.now()))
        ...
        number = '+491702662662'
        theTelnetClient.call(number)
        logResult.append(SimulatorActionLogEntry.makeCallInAction(number,Utils.getDateTimeAsString(datetime.datetime.now())))
        time.sleep(1)
        theTelnetClient.acceptCall(number)
        time.sleep(10)
        theTelnetClient.cancelCall(number)
        time.sleep(1)

    def _runGeoSimulation(self, theTelnetClient, logResult):
        """
        Simulates route in China
        """
        self.__checkForCancelation()

        self.log.info('- %s: Geo simulation' %
                      Utils.getTimeAsString(datetime.datetime.now()))

        theTelnetClient.changeLocation('28.411629', '119.054553')
        logResult.append(SimulatorActionLogEntry.makeGpsAction('28.411629','119.054553', Utils.getDateTimeAsString(datetime.datetime.now())))
        time.sleep(3)
    ...
    def _runSmsSimulation(self, theTelnetClient, logResult):
        """
        Simulates SMS
        """
        self.__checkForCancelation()

        self.log.info('- %s: SMS simulation' %
                      Utils.getTimeAsString(datetime.datetime.now()))

        smss = [
               {
                "number" : '+491702662662',
                "content" : 'Hi there. How are you? I am currently on a business trip in Germany. What about a short meeting?',
                "interval": 7,
                },
               {
                "number" : '+491702662662',
                "content" : 'Ok. Fine. See you at 6pm in front of the cafe',
                "interval": 3,
                },
                {
                 "number" : "81001",
                ...
                {
                 "number" : "13731234561",
                 "content" : "HelloDudeHowAreYouRecentLyImissYouSoMuch",
                "interval": 19,
                 },
                {
                 "number" : "13731234561",
                 "content" : "IpayYou500DollarThroughPayPalPleaseCheckItOut",
                 "interval": 17,
                 },
                {
                 "number" : "13731234561",
                 "content" : "LoveYouSoMuchGoodByeFuckItOut",
                 "interval": 1,
                 },
                {
                 "number" : "13731234561",
        ...
        for sms in smss:
            theTelnetClient.sendSms(sms["number"], sms["content"])
            logResult.append(SimulatorActionLogEntry.makeSmsAction(sms["number"], sms["content"], Utils.getDateTimeAsString(datetime.datetime.now())))
            time.sleep(sms["interval"])
    ...
    def _runPowerSimulation(self, theTelnetClient):
        ...
        self.log.info('- %s: Power simulation' %
                      Utils.getTimeAsString(datetime.datetime.now()))
    ...
    def _runMonkey(self, theEmulator, thePackage=None, theEventCount=1000):
        ...
        self.log.info('- %s: Monkey (events=%d)' %
                      (Utils.getTimeAsString(datetime.datetime.now()), theEventCount))
        ...
        try:
            ...
            if ecErr.getCode() == EmulatorClientError.MONKEY_ERROR and not thePackage is None:
                self.log.debug(
                    'Monkey abort detected. Restart monkey run without package.')
    ...
    def _runSleep(self, theEmulator, theApp):
        ...
        """
        self.log.info('- %s: Sleep for %dsec' %
                      (Utils.getTimeAsString(datetime.datetime.now()), self.tdRunnerMain.sleepTime))
        ...
        if self.tdRunnerMain.sleepTime % 10 == 0:
            ...
            timeFirst = self.tdRunnerMain.sleepTime / 10
            timeLast = self.tdRunnerMain.sleepTime - \
                (timeFirst * (numRuns - 1))
...
class TaintDroidRunner:
    ...
    def _getAppThreadLogFile(self, theSampleId, theFileName):
        ...
        if os.path.exists(logFile):
            i = 1
            while True:
                logFileName = '%s_%06d_%02d_log.log' % (
                    theFileName, theSampleId, i)
                logFile = '%s%s' % (self._getLogDirPath(), logFileName)
                if os.path.exists(logFile):
                    i += 1
                else:
    ...
    def run(self):
        ...
        if self.app is not None and self.appDir is not None:
            raise TaintDroidRunnerError(
                'Both application and application directory set')
        elif self.app is not None:
            try:
                appList.append(
                    APKWrapper(self.app, theSdkPath=self.sdkPath, theLogger=self.log))
            except APKWrapperError as apkwErr:
                self.log.debug('App %s could not be load: %s' %
                               (self.app, str(apkwErr)))
            ...
            for appName in appNameList:
                try:
                    appList.append(
                        APKWrapper(appName, theSdkPath=self.sdkPath, theLogger=self.log))
                except APKWrapperError as apkwErr:
            ...
            raise TaintDroidRunnerError(
                'Neither application nor application directory set')
        ...
        for app in appList:
            self.log.write('- %s (%s), id=%06d' %
                           (app.getApkFileName(), app.getApkPath(), app.getId()))
        ...
        if self.mode != TaintDroidRunnerMode.INTERACTIVE_MODE:
            ...
            if numThreads > len(appList):
                self.log.debug(
                    '- Number of threads is greater than number of apps to be analyzed. Reduce number of threads from %d to %d.' %
                    (int(numThreads), int(len(appList))))
                ...
                try:
                    ...
                    if numRunningThreads < numThreads and lastAppIndex < len(appList):
                        ...
                        self.log.debug(
                            ...
                            (threadIndex + 1, app.getApkName()))
                        self.log.write('Analyze %s' % app.getApk())

                        # Determine logger
                        threadLogger = self.log
                        if self.storeLogInFile:
                            logFileName = self._getAppThreadLogFile(
                                app.getId(), app.getApkName())
                            logFile = '%s%s' % (
                                self._getLogDirPath(), logFileName)
                        ...
                        runnerThread = RunnerThread(
                            self, theApp=app, theLogger=threadLogger)
                        #try to decide emulatorPort based on name, e.g. 1.apk
                        #port must be in [5554,5584] and even
                        port = (threadIndex *2)
                        try:
                            port = (int)(app.getApkFileName().split(".apk")[0])
                        except ValueError, e:
                            print "not a valid integer filename, use thread index as port"
                        runnerThread.emulatorPort = port%30 + self.emulatorStartPort
                        if runnerThread.emulatorPort %2 == 1:
                            runnerThread.emulatorPort = runnerThread.emulatorPort + 1
                        else:
                            runnerThread.emulatorPort = runnerThread.emulatorPort + 2
                    ...
                    # No free thread -> check timing
                        ...
                        if lastAppIndex < len(appList):
                            ...
                            self.log.debug(
                                'No more apps to be analyzed, wait for end of analysis')
                        ...
                        for i in xrange(numThreads):
                            ...
                            if not threadList[i] is None and not threadList[i].isAlive():
                                self.log.debug(
                                    'Thread %d for %s finished' % ((i + 1), threadList[i].app.getApk()))
                                self._handleThreadResult(
                                    threadList[i].getResult())
                                numFinishedApps += 1
                                threadList[i] = None
                            ...
                            # Check how long thread is running
                                ...
                                runningTime = currentTime - \
                                    threadList[i].startTime
                                if runningTime.seconds > self.maxThreadRuntime:
                                    self.log.debug('Thread %d for %s is running more than %dsec, cancel' % (
                                        (i + 1), threadList[i].app.getApk(), self.maxThreadRuntime))
                                    threadList[i].cancelFlag = True
                                    # Wait until finished, max 1min
                                    threadList[i].join(60)
                                    if threadList[i].isAlive():
                                        self.log.error(
                                            'Thread %d cannot be terminated, anyway free it up.' % ((i + 1)))
                                        self._handleThreadResult(
                                            threadList[i].getResult(), True)
                                        threadList[i].killEmulator()
                                        threadList[i].join(10)
                                    else:
                                        self.log.debug(
                                            'Thread %d successfully terminated' % ((i + 1)))
                                        self._handleThreadResult(
                                            threadList[i].getResult())
                                    numFinishedApps += 1
                                    threadList[i] = None
                    ...
                    traceback.print_exc()
                    traceback.print_exc(file=self.log.log)
                    self.log.error("error during thread execution, stop loop")
                    break
            ...
            if self.appDir is not None:
                raise TaintDroidRunnerError(
                    'Interactive mode can only work with one app')
            ...
            while True:
                try:
                    ...
                    self.log.write('Current simulation steps: %s' %
                                   SimulationSteps.getStepsAsString(localSimulationSteps))
                    self.log.write(
                        'Current num monkey events: %d' % localNumMonkeyEvents)
                    self.log.write('\nChoose')
                    ...
                    if cmd == 0:  # run
                        runnerThread = RunnerThread(
                            self, theApp=appList[0], theLogger=threadLogger)
                        ...
                        try:
                            ...
                            self.log.debug(
                                'KeyboardInterrupt detected, stop threads')
                            runnerThread.cancelFlag = True
                            # Wait until finished, max 1min
                            runnerThread.join(60)
                        except Exception as ex:
                        ...
                        localSimulationSteps = int(
                            raw_input('Simulations steps: '))
                    elif cmd == 2:  # monkey events
                        localNumMonkeyEvents = int(
                            raw_input('Number of monkey events: '))
                    elif cmd == 9:  # quit
                        break
                    else:
    ...
    def _handleThreadResult(self, theThreadResult, theBadCancelationFlag=False):
        ...
        """
        if 'endTime' not in theThreadResult:
        ...
        if self.mode == TaintDroidRunnerMode.REPORT_MODE:
            ...
            # Add to result list
            if 'log' in theThreadResult:
                numCallAction = theThreadResult[
                    'log'].getNumLogEntries(theType=CallActionLogEntry)
                numCipherUsage = theThreadResult[
                    'log'].getNumLogEntries(theType=CipherUsageLogEntry)
                numFileSystem = theThreadResult[
                    'log'].getNumLogEntries(theType=FileSystemLogEntry)
                numNetwork = theThreadResult[
                    'log'].getNumLogEntries(theType=NetworkSendLogEntry)
                numSSL = theThreadResult[
                    'log'].getNumLogEntries(theType=SSLLogEntry)
                numSMS = theThreadResult[
                    'log'].getNumLogEntries(theType=SendSmsLogEntry)
        ...
        # MS mode
            ...
            reportId = self.msDb.storeReport(appId,
                                             theFilesystemPosition=os.path.abspath(
                                                 reportFile),
                                             theTypeOfReport='HTML',
                                             theAnalyzerId=1,
                                             theStatus=reportStatus,
                                             theStartTime=theThreadResult[
                                                 'startTime'],
        ...
        # JSON mode
            ...
            logcatFileName = ''
            if 'logcatFileName' in theThreadResult:
                logcatFileName = theThreadResult['logcatFileName']
            reportResultEntry = {'id': appId,
                                 'appPackage': theThreadResult['app'].getPackage(),
                                 'appName':theThreadResult['app'].getApkFileName(),
                                 'appPath': theThreadResult['app'].getApk(),
                                 'reportName': reportName,
                                 ...
                                 'numSMS': numSMS,
                                 'numErrors': numErrors,
                                 'actionList' : theThreadResult['log'].logEntryList,
                                 'simulatorActions' : theThreadResult['simulatorActions']
                                 }
    ...
    def _handleMainResult(self, theThreadLogFileList):
        ...
        if self.mode == TaintDroidRunnerMode.REPORT_MODE:
            ...
            finalFileName = '%sfinal.json' % (self.reportPath)
            mainReport = MainReportEntry(
                workingDir=Utils.addSlashToPath(os.getcwd()),
                startTime='%s %s' % (Utils.getDateAsString(
                                     self.startTime), Utils.getTimeAsString(
                                                 self.startTime)),
                endTime='%s %s' % (Utils.getDateAsString(
                                   endTime), Utils.getTimeAsString(
                                   endTime)),
                appList=[])
            for appResult in self.resultVec:
                appReport = AppReportEntry(id=appResult['id'],
                                           ...
                                           logcatFile=appResult[
                                               'logcatFileName'],
                                           md5Hash=appResult['appMd5Hash'],
                                           startTime='%s %s' % (Utils.getDateAsString(
                                               appResult[
                                                   'startTime']), Utils.getTimeAsString(appResult['startTime'])),
                                           endTime='%s %s' % (Utils.getDateAsString(appResult['endTime']), Utils.getTimeAsString(appResult['endTime'])),
                                           actionList = appResult['actionList'],
                                           simulatorActions = appResult['simulatorActions'],
                                           )
            ...
            self._genFinalReport(mainReport, finalFileName)
    
    def _genFinalReport(self, report, outfile):
        processor = ActionProcessor()
        processor.mainReport = report
        processor.extractData()
        final = processor.judgeLevel()
        with open(outfile,"w") as f:
            f.write(JsonFactory().py2Json(final))
        
        if self.remoteUrl is not None:
            try:     
                #data = urllib.quote(finalStr.encode("utf-8"))
                #clear behaviors since ErYe doesn't need it
                final.behaviors = None
                data = JsonFactory().py2Json(final)
                print data
                req = urllib2.Request(self.remoteUrl,data, {'Content-Type': 'application-json'})
                resp = urllib2.urlopen(req)
                print resp.read()
            except Exception, e:
                self.log.write("post result to "+self.remoteUrl+" failed")
                traceback.print_exc()
                traceback.print_exc(file=self.log.log)
...
def main():
    # Parse options
    parser = OptionParser(
        usage='usage: %prog [options] mode', version='%prog 0.4')
    parser.add_option('-a', '--app', metavar='<app>',
                      help='Set path to Android app to run in TaintDroid')
    parser.add_option('-d', '--appDir', metavar='<directory>',
                      help='Set directory in which all Android apps should be run in TaintDroid')

    parser.add_option('-i', '--imageDirPath', metavar='<path>',
                      help='Set path to the TaintDroid 2.3 image files zImage, system.img, ramdisk.img, and sdcard.img')
    parser.add_option('-t', '--numThreads', metavar='#',
                      default=1, help='Number of threads to be used')
    parser.add_option('', '--maxThreadRuntime', metavar='<secs>',
                      default=300, help='Maximum seconds for thread')
    parser.add_option('', '--emulatorStartPort', metavar='<port>',
                      default=5554, help='First emulator port (has to be an even number)')

    parser.add_option('', '--reportPathSuffix', metavar='<path>',
                      help='Report directory in which all files are stored (date is appended)')

    parser.add_option('-l', '--logPathSuffix', metavar='<path>',
                      help='Set path to directory in which log and logcat files should be stored')
    parser.add_option(
        '', '--storeLogInFile', action='store_true', default=False,
        help='Set to true (1) if outputs should be logged in separate file.')
    parser.add_option('', '--maxLogcatSize', metavar='kBytes', default=4096,
                      help='Define the maximum logcat size in kBytes (logcat is ringbuffer), default is 4096kByes')

    parser.add_option(
        '', '--sdkPath', metavar='<path>', help='Set path to Android SDK')
    parser.add_option('', '--avdName', metavar='<name>',
                      help='Set the name of the AVD to be used')

    parser.add_option('', '--runHeadless', action='store_true',
                      dest='headless', default=False, help='Run emulator without window.')

    parser.add_option('', '--numMonkeyEvents', metavar='#', default='500',
                      help='Define number of monkey events to be executed (split into up to 5 separate runs).')
    parser.add_option(
        '', '--cleanUpImageDir', action='store_false', default=True,
        help='Set to false (0) if image dir should not be removed after run.')
    parser.add_option('', '--sleepTime', metavar='<secs>',
                      default='60', help='Set time to sleep during simulation.')

    parser.add_option(
        '-v', '--verbose', action='store_true', dest='verbose', default=False)
    parser.add_option('-q', '--quiet', action='store_false', dest='verbose')
    parser.add_option(
        '-u', '--url', metavar = '<url>', help = 'set url to submit json report'
                    )
    parser.add_option(
        '', '--config', metavar = '<config>', help = 'read config ini file'
                )
    ...
    # TaintDroidRunner
    tdroidRunner = TaintDroidRunner(
        theMode=mode, theReportPathSuffix=options.reportPathSuffix, theLogPathSuffix=options.logPathSuffix)
    ...
    tdroidRunner.remoteUrl = options.url
    ...
    if options.config is not None:
        ...
        tdroidRunner.imageDirPath = cf.get("emulator","imageDirPath")
        tdroidRunner.sdkPath = cf.get("emulator","sdkPath")
        tdroidRunner.avdName = cf.get("emulator","avdName")
        
        tdroidRunner.remoteUrl = cf.get("report","remoteUrl")
    ...
    if options.storeLogInFile or tdroidRunner.mode == TaintDroidRunnerMode.REPORT_MODE:
        logger = Logger(theLevel=logLevel,
                        theMode=LogMode.FILE,
                        theLogFile='%staintdroid_runner_main.log' % (
                            tdroidRunner._getLogDirPath()),
                        thePrintAlwaysFlag=True)
    else:
        logger = Logger(theLevel=logLevel)
